...
def planner(state: ResearchState) -> ResearchState:
    ...
    planner_model = model.with_structured_output(QueryPlanOutput)
...
def synthesis_cite(state: ResearchState) -> ResearchState:
...
def quality_checker(state:ResearchState) -> ResearchState:
    ...
    quality_model = model.with_structured_output(QualityCheckOutput)
...
def create_graph(model_provider: str="ollama", model_name:str='qwen3:8b'):
    ...
    graph.add_node("search_gather", search_gather)
    #graph.add_node("tool_node", tool_node)
    graph.add_node("synthesis_cite", synthesis_cite)
